 ...
 * Le shell (header + bandeau + sidebar + footer) est construit une seule fois,
...
const ASSETS = 'assets/';
const CACHE_KEY = 'nosgoth_layout_v2';

const GAMES = [
  { href: 'index.html',         label: 'Accueil',       logo: ASSETS + 'logo-original.webp', isHome: true },
  { href: 'blood-omen.html',    label: 'Blood Omen',    img: ASSETS + 'BO1.webp' },
  { href: 'soul-reaver.html',   label: 'Soul Reaver',   img: ASSETS + 'SR_og.webp' },
  { href: 'soul-reaver-2.html', label: 'Soul Reaver 2', img: ASSETS + 'SR2_og.webp' },
  { href: 'blood-omen-2.html',  label: 'Blood Omen 2',  img: ASSETS + 'BO2.webp' },
  { href: 'defiance.html',      label: 'Defiance',      img: ASSETS + 'Def_og.webp' },
  { href: 'ascendance.html',    label: 'Ascendance',    img: ASSETS + 'Asc.webp', cls: 'asc' },
];
...
// Note: header active state is NOT cached — set dynamically each page
function buildHeader(currentPage) {
  const items = GAMES.map(g => {
    const isActive = g.href === currentPage;
    let icon = '';
    if (g.isHome) icon = '';
    else if (g.img) icon = `<img src="${g.img}" alt="${g.label}">`;
    const cls = ['hitem', g.cls, isActive ? 'active' : ''].filter(Boolean).join(' ');
    return `<a href="${g.href}" class="${cls}">${icon}${g.label}</a>`;
  }).join('');
  return `<nav id="site-header"><div class="hinner">${items}</div></nav>`;
}
...
function buildFooter() {
  ...
  <div class="fcopy">
    ...
    <span class="fcopy-txt">Site non-officiel · Legacy of Kain © Crystal Dynamics / Square Enix · Royaume de Nosgoth © 2001–2026</span>
  </div>
</footer>`;
}
...
function injectLayout() {
  ...
  // Header always rebuilt (active state changes per page)
  siteBody.insertAdjacentHTML('beforebegin', buildHeader(currentPage));
